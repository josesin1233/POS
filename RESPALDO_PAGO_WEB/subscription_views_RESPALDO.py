...
def process_payment(request):
    ...
    try:
        ...
        # Mock registration for testing (bypass database)
        if not registration_id or not registration_id.startswith('REG_'):
            return JsonResponse({
                'success': False,
        ...
        registration = SimpleNamespace(
            id=registration_id,
            selected_plan=SimpleNamespace(price=299.00, name='estandar'),  # default values
