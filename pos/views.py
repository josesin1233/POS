...
class ProductoAPIView(View):
    ...
    def post(self, request):
        try:
            ...
            query = data.get('query', '').strip()  # Cambiar a 'query' genérico
            
            if not query or len(query) < 2:  # Necesita al menos 2 caracteres
                return JsonResponse({'productos': []})
            
            # Para simplificar, usar business ID 1 por defecto
            business_id = 1
            producto = None
            
            # Lógica inteligente: código corto vs nombre largo
            if len(query) <= 3:
                # Búsqueda por código (exacta)
                producto = Producto.objects.filter(
                    business_id=business_id,
                    codigo__iexact=query
                ).first()
            else:
...
class AgregarProductoView(View):
    ...
    def post(self, request):
        try:
            ...
            # Buscar producto
            if codigo:
                producto = Producto.objects.filter(business_id=business_id, codigo=codigo).first()
            elif nombre:
                producto = Producto.objects.filter(business_id=business_id, nombre__iexact=nombre).first()
            else:
